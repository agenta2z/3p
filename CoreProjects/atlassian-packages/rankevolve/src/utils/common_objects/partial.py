        ...
        if args and self._first_arg_values_to_param_map:
            ...
            if isinstance(args0, Mapping):
                for src, dst in self._first_arg_values_to_param_map.items():
                    _had_dst = dst in kwargs
                    if src in args0 and dst not in kwargs:
                        kwargs[dst] = args0[src]
                    # ROUND_TRACE: log when parts_subfolder is auto-extracted from type
                    if dst == "parts_subfolder":
                        import logging as _logging
                        _logging.getLogger("Partial").info(
                            "ROUND_TRACE Partial._FIRST_ARG: src=%s, dst=%s, "
                            "src_in_args0=%s, dst_was_in_kwargs=%s, "
                            "args0[src]=%s, final_kwargs[dst]=%s",
                            src, dst,
                            src in args0, _had_dst,
                            args0.get(src, "N/A") if isinstance(args0, Mapping) else "N/A",
                            kwargs.get(dst, "N/A"),
                        )
