...
from rankevolve.src.utils.common_utils.function_helper import (  # noqa: F401 — re-exported
    FallbackMode,
)
...
async def async_execute_with_retry(
    ...
    *,
    ...
    Args:
        ...
        non_retryable_exceptions: Tuple of exception types that ALWAYS propagate
            immediately, even if they would otherwise match ``retry_on_exceptions``.
            Use for deterministic-failure exceptions where retrying with the same
            input is guaranteed to fail again (e.g. ``asyncio.LimitOverrunError``
            from a subprocess emitting too-long lines, or
            ``InferencerExecutionError`` from a known-bad inferencer state).
            Default: ``()`` (disabled — every retry_on_exceptions hit retries).
    ...
    for chain_idx, current_func in enumerate(callable_chain):
        ...
        for attempt in range(current_max_attempts):
            ...
            try:
                ...
                # Short-circuit: ``non_retryable_exceptions`` always propagate
                # immediately, even when they would otherwise match
                # ``retry_on_exceptions``.  Used for deterministic failures
                # where retrying with the same input cannot succeed (e.g.
                # ``asyncio.LimitOverrunError`` from a subprocess line buffer
                # overflow — the same line will overflow again).
                if non_retryable_exceptions and isinstance(e, non_retryable_exceptions):
                    logging.getLogger(__name__).info(
                        "Non-retryable exception %s — propagating immediately "
                        "without retry: %s",
                        type(e).__name__,
                        str(e)[:200],
                    )
