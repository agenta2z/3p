...
def _split_(
    ...
    """
    Internal helper to split string.

    Inlined to avoid circular dependency on string_utils.
    """
    if rsplit_mode:
        parts = text.rsplit(sep, maxsplit) if maxsplit >= 0 else text.rsplit(sep)
    else:
...
def resolve_spaced_key_to_tuple(
    ...
    # Case 3: String input - split from right using internal _split_
    key_parts = _split_(
        key,
        sep=sep,
        maxsplit=1,
