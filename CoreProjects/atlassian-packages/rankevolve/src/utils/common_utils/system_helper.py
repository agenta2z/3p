...
Provides OS detection (``OperatingSystem`` / ``get_current_platform``) and
ARG_MAX utilities (``get_arg_max`` / ``get_available_arg_space``) used by
terminal-based inferencers to guard against ``E2BIG`` errors.
"""
...
def get_current_platform(
    ...
    Args:
        identify_mobile_operating_system: If True (default), attempts additional
            heuristics to distinguish Android or iOS from generic 'Linux' or
            'Darwin' outputs.
        system_str: A custom string to mimic the output of
            ``platform.system().lower()``. If None, calls ``platform.system()``.
        platform_str: A custom string to mimic the output of
            ``platform.platform().lower()``. If None, calls ``platform.platform()``.

    Returns:
        An ``OperatingSystem`` enum value representing the detected OS.
    ...
    """
    system_str = (system_str or platform.system()).lower()
    platform_str = (platform_str or platform.platform()).lower()
