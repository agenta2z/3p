...
from rankevolve.src.utils.io_utils.json_io import iter_json_objs
...
class SessionLogger:
    ...
    def from_existing(
        ...
        """Reopen an existing session log directory for resume.

        Unlike ``__init__``, this does NOT create a new timestamped directory
        or a new manifest. It reads the existing manifest to recover turn
        state and continues logging into the same directory.

        Args:
            session_dir: Path to the existing session directory.
            turn_aware_loggers: Optional loggers (same as __init__).
            cross_turn_loggers: Optional loggers (same as __init__).
            new_turn_log_type: Log type that triggers new turns.
            manifest_filename: Name of the manifest file.

        Returns:
            A SessionLogger that continues from where it left off.
        """
        manifest_path = Path(session_dir) / manifest_filename
        manifest = SessionManifest.from_json(manifest_path.read_text(encoding="utf-8"))

        instance = object.__new__(cls)
        instance._lock = threading.RLock()
        instance._session_id = manifest.session_id
        instance._session_type = manifest.session_type
        instance._new_turn_log_type = new_turn_log_type
        instance._manifest_filename = manifest_filename
        instance._session_dir = Path(session_dir)
        instance._manifest = manifest
        # Reset status to running since we're resuming
        instance._manifest.status = "running"
        instance._manifest.end_timestamp = None
        instance._turn_aware_loggers = list(turn_aware_loggers or [])
        instance._cross_turn_loggers = list(cross_turn_loggers or [])
        instance._current_turn_number = len(manifest.turns)
        instance._turn_step_counter = 0

        # Write updated manifest (status back to running)
    ...
    def _advance_turn(self) -> None:
        ...
        # Persist manifest so the turn boundary survives crashes
        self.write_manifest()
