...
# Canonical home is `agentic_foundation/common/workflow_constants.py`.
# Re-exported here for backward compatibility with any existing server-side callers.
...
__all__ = ["_WORKFLOW_DESC_PHASE_RE"]
...
def load_workflow_description(strategy: str, templates_dir: str = "") -> str:
    ...
    if not templates_dir:
        templates_dir = str(
            os.path.join(
                os.path.dirname(__file__), "..", "resources", "prompt_templates"
            )
        )
...
class WorkflowContext:
    ...
    # Task queue — tracks async tool invocations (task, understand_codebase).
    ...
    def complete_phase(
        ...
        **outputs,
        ...
        # Store outputs in phase_outputs so SOP tracker can find them
        if outputs:
    ...
    def get_next_runnable(self) -> dict[str, Any] | None:
        ...
        Queue-isolation rule (Section 5 M1 fix in the design doc):
        ``submission_run`` entries — long-lived ``buck run`` launches that
        can take 10-20 min for first-time builds — do NOT count against
        the PTI concurrency cap, AND a queued ``submission_run`` is
        allowed to start even while PTI is at cap. Without this rule,
        a single in-flight submission would block ALL hub PTI work for
        the duration of the buck build.
        """
        # First, look for the first queued entry. Submission runs bypass
        # the cap; PTI-class entries respect it.
        for entry in self.task_queue:
            if entry["status"] != "queued":
                continue
            # Implhyp wrapper + per-batch entries live in task_queue
            # purely for restoration on session_init reconnect; status
            # updates flow through ImplementHypothesisBridge callbacks
            # (see tool_executor.py:_on_batch_status), NOT through this
            # dispatcher. Skip them so the dispatcher never tries to
            # execute them as if they were regular tasks.
            if entry.get("tool_name") in (
                "implement_hypothesis",
                "implement_hypothesis_batch",
            ):
                continue
            if entry.get("tool_name") == "submission_run":
                return entry
            if self.running_count >= self.max_parallel_tasks:
                # PTI is at cap — but a SUBSEQUENT submission_run is
                # still eligible. Keep scanning.
                continue
            return entry
        return None
    ...
    def running_count(self) -> int:
        """Count of running PTI-class tasks. Excludes submission_run
        entries — they bypass the parallelism cap (see
        ``get_next_runnable``). Also excludes implhyp wrapper +
        per-batch entries — they're owned by ImplementHypothesisBridge
        (running inside the bridge's BTA), not the dispatcher, so they
        must not count against ``max_parallel_tasks``."""
        return sum(
            1
    ...
    def update_entry(self, task_id: str, **fields: Any) -> None:
        """Update arbitrary fields on a task_queue entry. Public API used by
        Layer 2 reconciliation and direct workspace assignment from _exec_task,
        avoiding ad-hoc mutation of the internal task_queue list."""
    ...
    def close_multi_task(self, multi_task_id: str) -> None:
        ...
        Use this instead of implicit auto-clear on phase completion — "all
        current runs done" does NOT mean "hub closed for new runs"
        (the multi-round Selection View flow adds more runs to a complete hub)."""
    ...
    def to_status_text(
        ...
        Args:
            ...
            sop_obj: P5 — optional ``SOP`` instance. When provided, an extra
                ...
                like Phase 1b/2b never become ``current_phase`` (they have
                no tool to fire ``start_phase()``) and are otherwise
                invisible in the status block. Without this the LLM sees
                "Current phase: Phase 1 (completed)" forever and assumes
                Phase 2 is next.
        """
        if (
        ...
        # P5: Next pending phase (esp. gate phases that have no tool to
        ...
        if sop_obj is not None:
            try:
                ...
                logger.warning("to_status_text P5 next-pending render failed: %s", e)
