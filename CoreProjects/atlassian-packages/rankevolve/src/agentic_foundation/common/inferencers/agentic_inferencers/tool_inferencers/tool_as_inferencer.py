...
subclass body is ~150 LOC of subprocess plumbing:
  ...
  - placeholder substitution + shell-metachar scrub via
    :mod:`rankevolve.src.common.streaming.cmd_helpers`
...
The plan-mode blueprint is at
``/home/zgchen/.claude/plans/humming-tinkering-wirth.md`` §1.1.
"""
...
from rankevolve.src.common.streaming.cmd_helpers import render_argv
...
# Mirrors ``submission_runner._BUILD_CMD_ALLOWLIST`` for the build-cmd allowlist
# plus the two new CLIs from this plan. Caller-overridable via the
# ``allowed_binaries`` field.
...
# Matches ``submission_runner._MAX_STREAM_LINE_BYTES``.
...
class ToolAsInferencer(StreamingInferencerBase):
    ...
    See module docstring + ``/home/zgchen/.claude/plans/…wirth.md §1.1``
    ...
    """Allowlist for the leading binary. Mirrors ``SubmissionRunner``'s
    posture: arbitrary ``bash`` is too easy to abuse, so each new binary
    ...
    def _build_argv(self, substitutions: Mapping[str, str]) -> list[str]:
        ...
        # Allow either a bare binary name ("rankevolve_train") OR an
        # absolute path whose basename matches an allowed binary.
    ...
    def _infer(
        ...
        **kwargs: Any,
        ...
        (e.g. :class:`DevmateSDKInferencer._infer`). Tools are usually
        invoked through ``ainfer``; this exists for the rare sync caller
        and to satisfy the ABC.
        """
    ...
    async def _terminate_proc(self, proc: asyncio.subprocess.Process) -> None:
        ...
        Mirrors :meth:`SubmissionRunner.cancel`. Safe to call multiple
        times (already-exited processes are no-ops). Never raises.
        """
    ...
    async def cancel(self) -> None:
        ...
        for parity with :meth:`SubmissionRunner.cancel` so callers that
        track per-tool cancellation explicitly have a uniform API.
        """
        proc = self._proc
        if proc is None:
            return
