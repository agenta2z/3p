...
from rankevolve.src.utils.path_utils.path_listing import (
...
class BreakdownThenAggregateInferencer(InferencerBase, WorkGraph):
    ...
    # Skip BOTH breakdown and workers — the aggregator runs once over
    # ``predefined_worker_results``. Use case: refresh a synthesis (e.g.
    # accumulated learnings) over per-worker outputs already on disk
    # without re-launching the underlying compute.
    # Implies skip-breakdown automatically; mutually exclusive with
    # ``breakdown_only`` and ``disable_aggregator``.
    disable_workers: bool = attrib(default=False, kw_only=True)
    # Verbatim worker outputs supplied by the caller when
    # ``disable_workers=True``. May be a list of strings (verbatim text)
    # OR a list of dicts ``{"path": "...", "summary": "..."}`` (the
    # aggregator's prompt builder embeds path refs when the aggregator
    # has ``has_local_access=True``, keeping the prompt small for many
    # large worker outputs).
    predefined_worker_results: Optional[List[Any]] = attrib(default=None, kw_only=True)
    ...
    def __attrs_post_init__(self):
        ...
        if self._workspace is not None:
            ...
            from rankevolve.src.agentic_foundation.common.inferencers.inferencer_workspace import (
                InferencerWorkspace,
            )
        ...
        # disable_workers validation — fail fast at construction time.
        if self.disable_workers:
            if self.predefined_worker_results is None:
                raise ValueError(
                    "disable_workers=True requires predefined_worker_results"
                )
            if self.disable_aggregator:
                raise ValueError(
                    "disable_workers=True is mutually exclusive with "
                    "disable_aggregator=True (nothing to do)"
                )
            if self.aggregator_inferencer is None:
                raise ValueError(
                    "disable_workers=True requires aggregator_inferencer"
                )
    ...
    def _build_diamond_graph(self, sub_queries, inference_config=None, **kwargs):
        ...
        for i, sq in enumerate(expanded_queries):
            ...
            # TODO(future migration): per-group concurrency limiting requires
            # WorkGraphNode in src/utils/common_objects/workflow/workgraph.py
            # to be migrated to accept a `group` kwarg, AND WorkGraph._arun's
            # fan-out semaphore logic to gain per-group support. Neither is in
            # place today — this variable is computed but currently unused
            # (see commented `# group=worker_group,` in the WorkGraphNode(...)
            # call below). Kept here as the future-work signal so a contributor
            # completing the migration knows where to re-wire it.
            worker_group = task_type if isinstance(self.worker_factory, dict) else None  # noqa: F841
            ...
            node = WorkGraphNode(
                ...
                # TODO(future migration): a newer BTA revision was migrated to
                # support per-group concurrency limiting via a `group` kwarg
                # on WorkGraphNode (e.g., MoE-style routing where workers of
                # the same task_type share a semaphore). The underlying
                # WorkGraphNode class in
                # rankevolve/src/utils/common_objects/workflow/workgraph.py
                # has NOT been updated to accept this kwarg, so passing it
                # raises `TypeError: WorkGraphNode.__init__() got an
                # unexpected keyword argument 'group'` and crashes the entire
                # BTA pipeline at construction time. Re-enable this line ONCE
                # WorkGraphNode is migrated AND WorkGraph._arun grows
                # per-group semaphore logic. For all current callers (research_propose,
                # experiment, implement_hypothesis), worker_factory is a callable
                # so `worker_group` is always None — the kwarg pass was a no-op
                # even on the intended runtime, so commenting it out has zero
                # functional impact today.
                # group=worker_group,
                # TODO(future migration): same WIP commit (72e8378ddf40) shipped
                # `enable_result_save=StepResultSaveOptions.SkipResumable` and
                # `resume_with_saved_results=ResumeMode.SkipResumable` to express
                # "let the worker manage its own save/resume; framework should
                # bypass". BUT the Resumable parent (resumable.py) typed both
                # attribs as `bool`, and the framework's runtime checks are
                # plain truthiness — `bool(ResumeMode.SkipResumable)` (a non-
                # empty str-enum value) is True, so the framework wrongly tried
                # to load/save and called the abstract `_get_result_path`,
                # raising NotImplementedError. Until the framework is migrated
                # to understand these enums, fall back to the framework
                # defaults (both False) — which produces the SAME effective
                # behavior the WIP intended (no framework-level save/resume;
                # worker PTIs handle it internally via their own checkpoints).
                # enable_result_save=StepResultSaveOptions.SkipResumable,
                # resume_with_saved_results=ResumeMode.SkipResumable,
                # TODO(future migration): same WIP commit (72e8378ddf40) that
                # added the `group=` kwarg above also added this
                # `worker_manages_resume=` kwarg. The underlying WorkGraphNode
                # in src/utils/common_objects/workflow/workgraph.py and its
                # parents (WorkNodeBase, Resumable) were NOT updated to accept
                # `worker_manages_resume`, so passing it raises
                # `TypeError: WorkGraphNode.__init__() got an unexpected
                # keyword argument 'worker_manages_resume'` and crashes BTA at
                # construction. The intent (per
                # `common/step_result_save_options.py` docstrings) is for nodes
                # whose worker manages its own checkpointing/resume to skip
                # the framework-level result save/resume — useful for nested
                # PTI workers (which already have their own checkpoints).
                # Re-enable this line ONCE the kwarg lands on WorkGraphNode
                # (and the framework's save/resume bypass logic is wired). For
                # now, the value `_worker_manages_resume` is computed at line
                # 813 from `isinstance(worker, Resumable) and worker.resume_with_saved_results`
                # and passed via the worker fn closure (line 934) — that
                # closure-side use is unaffected by this comment-out.
        ...
        if self.graph_reporter is not None and not self._graph_topology_emitted:
            ...
            try:
                from rankevolve.src.agentic_foundation.common.inferencers.graph_events import (
    ...
    def _resolve_aggregator_inferencer(self) -> InferencerBase:
        """Resolve self.aggregator_inferencer to an InferencerBase instance.

        If the field holds a callable (lazy factory), invoke it once and
        cache. Configures a child workspace under ``children/aggregator/``
        so logs and outputs land where downstream code expects.
        """
        agg_inf = self.aggregator_inferencer
        if callable(agg_inf) and not isinstance(agg_inf, InferencerBase):
            agg_inf = agg_inf()
    ...
    def _build_aggregator_only_input(
        self, worker_results: List[Any], original_query: Any
    ) -> Any:
        """Construct the aggregator's prompt for ``disable_workers`` mode.

        Mirrors the in-graph ``_build_agg_input`` closure used by the
        normal worker→aggregator path: prefer ``aggregator_prompt_builder``
        when supplied, otherwise build a default text. Path-only dicts
        (``{"path": "..."}``) are passed through as ``worker_output_paths``
        so the aggregator can read files lazily when ``has_local_access``.
        """
        # Extract path hints if the caller supplied them as dicts.
        worker_output_paths: List[Optional[str]] = []
        flattened_results: List[Any] = []
        for item in worker_results:
            if isinstance(item, dict) and "path" in item:
                worker_output_paths.append(str(item["path"]))
                flattened_results.append(item.get("summary") or item.get("path", ""))
            else:
                worker_output_paths.append(None)
                flattened_results.append(item)

        builder = self.aggregator_prompt_builder
        if builder is not None:
            try:
                ...
                try:
                    ...
                    return builder(flattened_results, original_query=original_query)

        # Default: same shape as the in-graph default.
        agg_inf = self.aggregator_inferencer
        agg_has_local = getattr(agg_inf, "has_local_access", False)
        parts: List[str] = []
        for idx, res in enumerate(flattened_results):
            path = worker_output_paths[idx] if idx < len(worker_output_paths) else None
            if agg_has_local and path:
    ...
    def _infer_aggregator_only(self, inference_input, inference_config=None):
        """Sync aggregator-only execution path. Bypasses WorkGraph.

        Construction-time validation in ``__attrs_post_init__`` guarantees
        ``aggregator_inferencer`` and ``predefined_worker_results`` are set.
        """
        agg_inf = self._resolve_aggregator_inferencer()
        agg_input = self._build_aggregator_only_input(
            list(self.predefined_worker_results or []), inference_input
        )
        if hasattr(agg_inf, "infer"):
            result = agg_inf.infer(agg_input, inference_config=inference_config)
        else:
            result = agg_inf(agg_input)
        self._finalize_response(result)
        return result

    async def _ainfer_aggregator_only(self, inference_input, inference_config=None):
        """Async aggregator-only execution path. Bypasses WorkGraph."""
        agg_inf = self._resolve_aggregator_inferencer()
        agg_input = self._build_aggregator_only_input(
            list(self.predefined_worker_results or []), inference_input
        )
        if hasattr(agg_inf, "ainfer"):
            result = await agg_inf.ainfer(
                agg_input, inference_config=inference_config
            )
        elif hasattr(agg_inf, "infer"):
            result = agg_inf.infer(agg_input, inference_config=inference_config)
        else:
            result = agg_inf(agg_input)
        self._finalize_response(result)
    ...
    def _infer(self, inference_input, inference_config=None, **kwargs):
        ...
        # Step 0a: Aggregator-only mode — bypass breakdown + workers + graph.
        if self.disable_workers:
            return self._infer_aggregator_only(
    ...
    async def _ainfer(self, inference_input, inference_config=None, **kwargs):
        ...
        # Step 0a: Aggregator-only mode — bypass breakdown + workers + graph.
        if self.disable_workers:
            return await self._ainfer_aggregator_only(
                inference_input, inference_config=inference_config
            )
