...
_DEVMATE_CONFIG_DIR: str = (
    "fbcode/rankevolve.src.agentic_foundation/common/inferencers"
...
class SessionMode(Enum):
    ...
    cached file reads, prior tool results — fast follow-ups) but accumulates
    devmate's per-session caps (``max_iterations``, ``max_time_mins``,
    ``max_total_tokens``) and risks state corruption persisting across calls.

    Class-level default on ``DevmateCliInferencer`` is
    ``SAME_SESSION_ACROSS_ROUNDS`` for backward-compat with non-dual-inferencer
    callers (chat_cli, single-shot uses) that depend on session continuity.
    Dual_inferencer ctor-sites override to ``NEW_SESSION_PER_CALL`` via the
    bridge factory (because each dual_inferencer round is conceptually
    independent and the prompts are self-contained — verified via
    prompt-self-containment audit, see velvet-wondering-crane plan F3 notes).
    """

    SAME_SESSION_ACROSS_ROUNDS = "same"
    """Reuse active_session_id across all calls. Today's behavior; backward
    compatible. Best for tightly-coupled multi-turn flows like chat_cli
    where session memory is high-value and per-session caps unlikely to bind.
    """

    NEW_SESSION_PER_CALL = "per_call"
    """Fresh session for every infer/ainfer call. Each call's prompt MUST
    be self-contained (verified true for all dual_inferencer callers). Best
    for orchestrator-driven loops where iteration accumulation is a risk.
    Recommended default for dual_inferencer ctor-sites."""

    NEW_SESSION_ON_ERROR = "on_error"
    """Reuse session on success; reset to fresh on any
    ``InferencerExecutionError``. Generalizes the existing
    ``_looks_like_tool_use_corruption`` recovery to a catchall. Best for
    callers that want session continuity on the happy path but immediate
    ...
    tunable via ``consecutive_error_threshold`` attrib). Tolerates one
    transient hiccup before assuming structural breakage."""
...
def get_source_repo_root() -> str:
    ...
    Walks up from this file to the fbsource root. This file is at:
        <fbsource>/fbcode/rankevolve.src.agentic_foundation/common/
