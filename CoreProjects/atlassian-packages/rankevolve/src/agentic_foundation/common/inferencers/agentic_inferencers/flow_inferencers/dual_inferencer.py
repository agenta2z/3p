...
from rankevolve.src.agentic_foundation.common.inferencers.agentic_inferencers.constants import (
...
from rankevolve.src.utils.common_objects.input_and_response import InputAndResponse
from rankevolve.src.utils.common_objects.serializable import SerializationMode
...
from rankevolve.src.utils.common_objects.workflow.common.step_wrapper import StepWrapper
from rankevolve.src.utils.common_objects.workflow.workflow import Workflow
from rankevolve.src.utils.string_utils.formatting.template_manager import (
    TemplateManager,
)
from rankevolve.src.utils.string_utils.xml_helpers import unescape_xml

logger = logging.getLogger(__name__)


def _looks_like_dict_repr(s: str) -> bool:
    """Detect a stringified Python dict repr (success OR failure).

    Inferencers may return plain ``dict`` objects (e.g. ``DevmateCliInferencer.
    parse_output`` returns ``{success, output, raw_output, ...}``). A caller
    that bypasses ``extract_response_text`` and writes ``str(some_dict)`` to
    disk corrupts the artifact: ``dict.__repr__`` escapes real newlines to
    literal ``\\n`` two-character sequences, collapsing the entire dict onto
    a single line in the saved markdown.

    Heuristic: starts with ``{`` and matches a dict-repr key prefix (an
    identifier — quoted single, double, or unquoted — followed by ``:``).
    Both Python repr (``'key':``) and JSON (``"key":``) shapes are caught.

    The previous narrower form (``_looks_like_failure_dict_repr``) caught
    only ``success: False`` failure dicts, leaving ``success: True``
    success-dicts unguarded — which is exactly how the artifact corruption
    in task_20260424_231308 happened.
    """
    if not s:
        return False
    head = s.lstrip()[:1]
    if head != "{":
        return False
    return bool(re.match(r"\s*\{['\"]?\w+['\"]?\s*:\s*", s))
...
class DualInferencer(InferencerBase, Workflow):
    ...
    def _save_result(self, result, output_path: str):
        from rankevolve.src.utils.common_utils.map_helper import dict__
    ...
    def _load_result(self, result_id, result_path_or_preloaded_result):
        from rankevolve.src.utils.io_utils.json_io import read_json
    ...
    def _build_consensus_steps(
        ...
        async def _step_propose_impl(*args, **kwargs):
            ...
            # inferencers (DevmateCli / ClaudeCodeCli) get unwrapped via their
            # ``output`` key instead of being stringified to a dict-repr — which
            # would escape real newlines to literal ``\n`` and corrupt the artifact
            # written by ``_maybe_replace_with_file_reference``.
            _raw_base = extract_response_text(
            ...
            if base_output_str is None:
                raise InferencerExecutionError(
                    ...
                    error=(
                        ...
                        "<Response> block (likely truncated by max_tokens or "
                        "deliberately skipped after a tool call). "
        ...
        async def _step_review_impl(*args, **kwargs):
            ...
            # See _step_propose_impl above for why we use ``extract_response_text``
            # instead of raw ``str()``.
            _raw_review = extract_response_text(
            ...
            if review_output_str is None:
                raise InferencerExecutionError(
                    ...
                    error=(
                        "response_parser returned None — reviewer emitted no clean "
                        "<Response> block (likely truncated by max_tokens or "
                        "deliberately skipped after a tool call). "
        ...
        async def _step_fix_impl(*args, **kwargs):
            ...
            # See _step_propose_impl above for why we use ``extract_response_text``
            # instead of raw ``str()``.
            _raw_fix = extract_response_text(
            ...
            self.log_debug(
                _raw_fix,
                # NOTE: label is "RawFollowupResponse" (NOT "RawFixResponse") to
                # mirror the post-extraction "FollowupResponse" label below — the
                # raw/extracted pair share the same role-name (followup), the
                # only difference is the "Raw" prefix indicating pre-extraction.
                "RawFollowupResponse",
                is_artifact=True,
                parts_min_size=0,
                parts_subfolder=f"Round{total_iters:02d}",
            ...
            if fix_output_str is None:
                raise InferencerExecutionError(
                    ...
                    error=(
                        "response_parser returned None — fixer emitted no clean "
                        "<Response> block (likely truncated by max_tokens or "
                        "deliberately skipped after a tool call). "
            ...
            # F0 — Convergence detection: when the fixer signals a structured
            # no-op (no items + summary starts with "no-op:"), the previous
            # round's output is canonical. Short-circuit the consensus loop
            # instead of spinning further degenerate rounds (which would
            # invite token-limit truncation, dropped <Response> tags, and
            # corrupt round files — see forensics_round_template_echo_bug).
            no_op_signal = not parsed_counter.get("items") and parsed_counter.get(
                "summary", ""
            ).lower().lstrip().startswith("no-op:")
            if no_op_signal:
                logger.info(
                    "[%s] F0_NO_OP_CONVERGENCE: fixer signaled no-op "
                    "(summary=%r); short-circuiting consensus loop without "
                    "updating base_output_str",
                    self.phase or "DualInferencer",
                    parsed_counter.get("summary", ""),
                )
                ...
                state["attempt_record"]["consensus_reached"] = True
                state["attempt_record"]["final_output"] = state["base_output_str"]
                state["attempt_record"]["final_feedback"] = state.get(
                    "parsed_review", {}
                )
                # Do NOT overwrite base_output_str — the previous round's content
                # remains the canonical output. Do NOT call
                # _maybe_replace_with_file_reference — there is no new file to write.
                self._state = state
    ...
    def _infer(self, inference_input, inference_config=None, **_inference_args):
        ...
        from rankevolve.src.utils.common_utils.async_function_helper import _run_async
    ...
    # Markers that indicate the matched content is a prompt-template echo
    # rather than the model's actual response. Both must be present to count
    # as a prompt-echo (avoids false-positives on legitimate output that
    # happens to mention "important" or quote prompt phrasing in passing).
    _PROMPT_ECHO_MARKERS: ClassVar[Tuple[str, ...]] = (
        "**IMPORTANT",
        "wrap your response",
    )
    ...
    def _default_response_parser(raw: str) -> Optional[str]:
        ...
        Three return modes:
        1. **No matches at all** → returns the raw string unchanged
           (legacy passthrough — model produced unstructured output).
        2. **One or more clean matches** → returns the most-recent clean
           match's content (stripped).
        3. **Matches exist but ALL are prompt-template echoes** → returns
           ``None``. Signals "model emitted no real <Response> block";
           callers should treat as hard failure rather than silently
           writing prompt-template content to disk
           (see forensics_round_template_echo_bug).
        """
        any_matches = False
        for tag in ("Response", "ImprovedProposal"):
            ...
            for match in reversed(matches):
                ...
                if all(
                    marker in content for marker in DualInferencer._PROMPT_ECHO_MARKERS
                ):
                    continue  # prompt-template echo, skip
                return content
        if not any_matches:
            # Legacy passthrough: no tags found at all.
            return raw
        # All matches across all tag-types were prompt-echo.
        return None
    ...
    def _maybe_replace_with_file_reference(
        ...
        **Legacy mode**: Uses ``inference_config["output_path"]`` template with
        ...
        Belt-and-braces: refuse to write a stringified Python dict-repr (any
        shape, success or failure) as if it were a real LLM response. Any
        caller that bypassed ``extract_response_text`` and stringified a dict
        via raw ``str()`` would otherwise corrupt the artifact (escaping real
        newlines to literal ``\\n``) and cause the reviewer loop to chase a
        phantom plan.
        """
        if _looks_like_dict_repr(response_str):
            logger.error(
                "[DualInferencer] Refusing to save dict-repr as plan output "
                "(response_str=%r...)",
                response_str[:200],
            )
            raise InferencerExecutionError(
                tool="<dual_inferencer.fallback>",
                error=(
                    "extract_response_text bypass: a Python dict-repr (success "
                    "or failure) reached _maybe_replace_with_file_reference as "
                    "a string. Likely cause: an upstream inferencer returned a "
                    "plain dict that was stringified via raw ``str(dict)`` "
                    "instead of unwrapped via ``extract_response_text`` — would "
                    "have written garbage with literal ``\\n`` escapes to "
                    f"round{round_index:02d}_{self.output_path or 'plan.md'}."
                ),
            )
        ...
        if self._workspace is not None and self.output_path:
            ...
            if os.path.isfile(resolved_path) and os.path.getsize(resolved_path) > 0:
                ...
                try:
                    ...
                    # Don't claim "the file is written" when the write failed.
                    # Returning the raw response keeps the LLM's content alive
                    # for the next stage, but the misleading "Read that file"
                    # message would lie to downstream reviewers.
                    logger.error(
        ...
        if os.path.isfile(resolved_path) and os.path.getsize(resolved_path) > 0:
            ...
            try:
                ...
                # Same rationale as workspace-mode above: keep the response
                # text intact for the next stage but DO NOT lie about a
