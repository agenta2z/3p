...
class ClaudeCodeCliInferencer(TerminalSessionInferencerBase):
    ...
    Usage Patterns:
        # Simple single-call:
        inferencer = ClaudeCodeCliInferencer(root_folder="/path/to/repo")
        result = inferencer("Write a hello world program")

        # Multi-turn with auto-resume:
        inferencer = ClaudeCodeCliInferencer(root_folder="/repo", auto_resume=True)
    ...
    Attributes:
        root_folder: Absolute path to the target repository/workspace where the
            ...
            Standardized name across all inferencer classes (was ``target_path``).
    ...
    def __attrs_post_init__(self) -> None:
        ...
        from rankevolve.src.agentic_foundation.common.inferencers.agentic_inferencers.external.claude_code.common import (
            resolve_model_tag,
        )
    ...
    async def _ainfer_streaming(self, prompt: str, **kwargs: Any) -> AsyncIterator[str]:
        ...
        try:
            ...
            # A single Claude CLI stdout line exceeded ``_MAX_STDOUT_LINE_BYTES``
            # (16 MB).  Each event line is one JSON object; if even that is
            # too large, we cannot safely chunk-parse it.  Log + truncate the
            # stream — partial text deltas already yielded are preserved.
            # The retry layer in InferencerBase treats ``LimitOverrunError``
            # as non-retryable (P4).
            #
            # NOTE: ``async for line in stream`` internally calls
            # ``readline()``, which catches ``LimitOverrunError`` and re-
            # raises as ``ValueError("Separator is not found, and chunk
            # exceed the limit")``.  We catch BOTH exception types so this
            # block fires regardless of how the buffer overflow surfaces.
            if isinstance(e, ValueError) and (
                "Separator is not found" not in str(e)
                and "chunk exceed the limit" not in str(e)
            ):
                # Unrelated ValueError (not a buffer overflow) — propagate.
                raise
            logger.warning(
                "[%s] Claude CLI stdout line exceeded %d-byte buffer; "
                "stream truncated. Partial deltas already yielded are "
                "preserved. (%s)",
                self.__class__.__name__,
                _MAX_STDOUT_LINE_BYTES,
                str(e)[:200],
