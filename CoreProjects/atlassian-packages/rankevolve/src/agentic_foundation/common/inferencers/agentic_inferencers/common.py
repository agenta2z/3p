...
from rankevolve.src.utils.string_utils.xml_helpers import mapping_to_xml
...
class InferencerExecutionError(RuntimeError):
    """Raised when a terminal-inferencer (Devmate / Claude Code CLI / Metamate
    CLI) returns a structured failure result.

    The legacy fall-through ``return str(result)`` in ``extract_response_text``
    silently turned ``{'success': False, 'error': '...port file...'}`` into a
    ``"{'success': False, ...}"`` string that downstream code wrote to the plan
    file as if it were a real LLM response. Surface failures as exceptions so
    callers can decide how to render them (chip ERROR, log, retry).
    """
...
class MaxIterationsExhaustedError(InferencerExecutionError):
    """Raised when devmate's per-session iteration counter is exhausted.

    Devmate's orchestrator caps iterations per session (``max_iterations``).
    The counter is per-session cumulative — calls reusing an existing
    session inherit the prior count. When a single ``ainfer()`` call reuses
    a session that already accumulated near the cap from prior calls, the
    call fails immediately with this error.

    This is NOT a code bug in the model or rankevolve — it's devmate
    session-state exhaustion. Surface as a distinct subclass so retry
    policy can react (e.g., start a new session via
    ``SessionMode.NEW_SESSION_ON_ERROR``) and user-facing errors can give
    the right diagnostic.

    Subclass of ``InferencerExecutionError`` to preserve backward
    compatibility with existing ``except InferencerExecutionError`` clauses.

    Attributes:
        max_iterations: The cap value reported by devmate (parsed from
            "Max iterations of N reached" via regex), or None if unparseable.
        session_id: The exhausted devmate session ID (so users can correlate
            with stream logs / trajectory inspector).
    """
...
def extract_response_text(result):
    ...
    - Plain dict without ``success`` key → returns the first matching string
      ...
      ``InferencerExecutionError`` for unrecognized dict shapes (refuses to
      silently stringify a dict because ``dict.__repr__`` escapes real
      newlines to literal ``\\n`` and corrupts downstream artifacts).
    - Other → str(result)
    ...
    if isinstance(result, dict) and "success" in result:
        # Terminal-inferencer (Devmate / Claude Code CLI / Metamate CLI)
        # contract per ``parse_output``: dict with success/output/error/...
        if not result.get("success", False):
            error_text = result.get("error", "") or ""
            # Detect MAX_ITERATIONS exhaustion (devmate's per-session
            # iteration cap) and raise the distinct subclass so callers
            # can react appropriately (e.g., start a new session via
            # SessionMode.NEW_SESSION_ON_ERROR). This covers the path
            # where parse_output returns the error dict instead of the
            # _ainfer error handler raising directly.
            if (
    ...
    if isinstance(result, dict):
        # Plain dict without ``success`` — accept common LLM-response shapes
        # (``output`` from terminal CLIs, ``raw_output`` as a fallback,
        # ``content`` from SDK responses).
        for key in ("output", "raw_output", "content"):
            value = result.get(key)
            if isinstance(value, str):
                return value
        # Unrecognized dict shape: surface loudly instead of silently doing
        # ``str(result)`` (which would invoke ``dict.__repr__`` and escape
        # real newlines to literal ``\n``, corrupting any artifact written
        # via ``f.write(extract_response_text(result))``).
        raise InferencerExecutionError(
