...
from rankevolve.src.utils.common_utils import dict_, iter__, resolve_environ
...
class InferencerBase(Debuggable, Resumable, ABC):
    ...
    def _redirect_loggers_to_workspace(self, workspace):
        ...
        from rankevolve.src.utils.io_utils.json_io import JsonLogger
    ...
    def __attrs_post_init__(self):
        ...
        if isinstance(self.response_post_processor, (str, dict)):
            _builtin_post_processors = {
                "extract_delimited": lambda: __import__(
                    "rankevolve.src.agentic_foundation.common.response_parsers.delimiter_parser",
        ...
        if isinstance(self.post_response_merger, str):
            if self.post_response_merger == "default":
                from rankevolve.src.utils.mp_utils.common import merge_results

                self.post_response_merger = merge_results
            else:
                from rankevolve.src.utils.mp_utils.common import get_merger
    ...
    def parallel_infer(
        ...
        **_inference_args,
        ...
        """
        # TODO(migration): port mp_target.py and parallel_process.py from
        # _tony_dev/CoreProjects/RichPythonUtils when needed by a real caller.
        try:
            from rankevolve.src.utils.mp_utils.mp_target import MPTarget
            from rankevolve.src.utils.mp_utils.parallel_process import (
                parallel_process_by_pool,
            )
        except ImportError as exc:
            raise NotImplementedError(
                "rankevolve.src.utils.mp_utils.parallel_process / mp_target not yet ported. "
                "Required for InferencerBase.parallel_infer / aparallel_infer. "
                "TODO(migration): port from _tony_dev/CoreProjects/RichPythonUtils when needed."
            ) from exc
        ...
        if num_workers is None:
            if use_threading:
                ...
                from rankevolve.src.utils.mp_utils.common import (
                    get_suggested_num_workers,
                )
    ...
    async def _ainfer_single(
        ...
        """
        from rankevolve.src.utils.common_utils.async_function_helper import (
        ...
        token = _current_fallback_state.set(_fallback_state)
        try:
            # ``non_retryable_exceptions``: short-circuit deterministic failures
            # so retries don't waste attempts re-issuing the same call.
            # - ``asyncio.LimitOverrunError`` / ``IncompleteReadError``: subprocess
            #   stream-buffer overflow on a single line.  The same overflowing
            #   line will reproduce on every retry against a deterministic
            #   subprocess (e.g. an LLM CLI re-emitting cached output).  P3
            #   already adds a chunked-read fallback in
            #   ``terminal_session_inferencer_base.py``, but this is the
            #   defense-in-depth filter for any remaining call site that lacks
            #   the fallback (e.g. ``claude_code_cli_inferencer.py``'s JSON
            #   stream which can't safely chunk-parse).
