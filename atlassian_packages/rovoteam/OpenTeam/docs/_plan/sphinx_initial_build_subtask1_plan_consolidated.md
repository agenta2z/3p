# Sphinx Initial Build — Subtask 1 (Install + Capture) — Consolidated Plan

**Plan author:** assistant (Claude), consolidating upstream artifacts
**Plan date:** 2026-05-04
**Repo:** `C:\Users\yxinl\OneDrive\Projects\PythonProjects\CoreProjects\OpenTeam`
**Subtask scope:** install Sphinx + RTD theme, run cold & warm builds against `docs/source/`, save raw logs and a version manifest. Hand off to **subtask 2** (warning triage / recovery). **Do not modify any existing file under `docs/source/`.**

---

## 0. Consolidation note

This artifact consolidates one upstream plan (`docs/_plan/sphinx_initial_build_subtask1_plan.md`, 513 lines) into a tighter, current-state-aware execution doc. A second parallel flow produced no output, so the consolidation is a **refinement of a single input plus newly-verified evidence**, not a multi-input merge.

**What changed vs. the upstream plan:**

1. **NEW EVIDENCE — system Python already has Sphinx installed.** Verified at consolidation time: `python` on PATH (`/c/Users/yxinl/miniforge3/python`, 3.12.7) imports `sphinx==8.2.3`, `sphinx_rtd_theme==3.0.2`, `docutils==0.21.2` cleanly. The upstream plan did not check this and went straight to a venv-creation path. This adds **Option 0** (use system Python as-is) below as the cheapest viable route, while keeping Option C (`python -m venv`) as the safer/more reproducible default.
2. **Updated page-count prediction.** Current `docs/build/html/` (the post-fix build that already exists) contains **7 HTML files** (`index, configuration, inference-models, operations, glossary, genindex, search`), not "~8 give or take" as the upstream plan estimated. Sharpened in §5.1.
3. **Tightened prose and de-duplicated cross-references**; collapsed upstream §1.5 / §11 redundancy into §2.5 + §12. Substance unchanged.
4. **All other upstream findings preserved verbatim** — discrepancy table (§3), install-option matrix (§4), failure matrix (§7), compliance checklist (§9), open questions (§11).

A small confession on integration value: with only one substantive input, this consolidation is mostly verification + the system-Python finding. See the iteration judgment at the end of this run.

---

## 1. TL;DR for the executor

This subtask is mechanically simple — install, build twice, save logs — but **two premises in the user request do not hold** in this repo, and **a third premise is already partially satisfied on disk**:

1. **"uv-native, per CONSOLIDATION_NOTES item #3 and pyproject.toml"** — neither `pyproject.toml` nor any `CONSOLIDATION_NOTES*` file exists at repo root, and `uv` is not on PATH. `uv add --dev …` will hard-fail. §4 picks the closest viable substitute.
2. **"do not modify ANY existing files (not the 8 prose pages, …)"** — only **3** prose pages exist on disk (`configuration.rst`, `inference-models.rst`, `operations.rst`) plus `index.rst` and `glossary.rst`. The "8 prose pages" wording is parent-spec leakage already flagged by `docs/_plan/sphinx_scaffolding_audit.md §4`. The "do not modify" constraint is honored against *whatever exists*.
3. **The build was already executed once.** `docs/build/build-log-warm.txt` exists and ends with `build succeeded, 81 warnings.` (Sphinx 8.2.3). A subsequent triage subtask landed fixes; `docs/build/build-log-after-fix.txt` ends with `build succeeded, 6 warnings.` This plan **re-runs the cold + warm capture from scratch** because the user request explicitly asks for fresh `build-log-cold.txt` + `build-log-warm.txt` artifacts and a fresh version manifest, *not* to consume the existing warm log.

**Deliverable set (per the user request):**
...
### 2.1 Disk inventory — `docs/source/`

```
docs/source/
├── _static/                       (per audit, .gitkeep only)
├── conf.py                        ← exists
├── index.rst                      ← exists, 41 lines, toctree at lines 22–34
├── configuration.rst              ← exists (foundation, read-only)
├── inference-models.rst           ← exists (foundation, read-only)
├── operations.rst                 ← exists (foundation, read-only)
└── glossary.rst                   ← exists (NEW file, line 13 already fixed)
```

The `.. toctree::` block at `index.rst:22-34` lists 9 page basenames; **5 are MISSING** on disk: `introduction`, `getting-started`, `architecture`, `service-layer`, `api-reference`. This is reproduced from `docs/_plan/sphinx_scaffolding_audit.md §4` and confirmed by direct listing. The 5 missing pages produce 5 toctree warnings on the warm build — that is **expected output**, not a build failure.

### 2.2 `conf.py` settings that materially shape build output

(Read at plan time — see `docs/source/conf.py` for full text.)
...
- `nitpick_ignore` block — 9 `('std:ref', '<anchor>')` tuples covering the 9 unique forward-referenced anchor names from the missing-page set, with per-tuple citation comments
...
### 2.3 Repo-level inventory (uv-premise check + system-Python check)
...
| `sphinx` importable from system Python    | **YES** — version `8.2.3`                   |
| `sphinx_rtd_theme` importable             | **YES** — version `3.0.2`                   |
| `docutils` importable                     | **YES** — version `0.21.2`                  |

Verified via `Glob` (no `**/*.toml` matches), direct `ls`, and direct `python -c "import …; print(__version__)"`. The `conf.py` header itself acknowledges the missing-pyproject state: *"No pyproject.toml is checked in alongside this docset."*

**The system-Python finding is the consolidation's main delta over upstream.** It changes the install decision: see §4.2 Option 0.
...
The `html/` listing today: `index.html, configuration.html, inference-models.html, operations.html, glossary.html, genindex.html, search.html` — exactly 7 `.html` files, plus `_sources/`, `_static/`, `objects.inv`, `searchindex.js`.

**Implication:** the directory is non-empty. To produce a clean cold build, the plan deletes `docs/build/html` *and* `docs/build/.doctrees` (if present) before the cold run. The two existing log files are **kept** under renamed paths — they are upstream evidence cited by `sphinx_warning_triage_plan_consolidated.md §1` and overwriting them would destroy provenance for the prior triage subtask.

The new logs land at `docs/build/build-log-cold.txt` and `docs/build/build-log-warm.txt`. The latter is a **filename collision** with the existing pre-triage log. Per §4.4, the plan renames the existing `build-log-warm.txt` to `build-log-warm-pre-triage.txt` before running, preserving its content under a clearly-distinct name.

### 2.5 Prior-art plans on disk (which this plan complements)

Listed once here (not duplicated in a separate cross-reference section):
...
- `docs/_plan/sphinx_initial_build_plan.md` (520 lines) — second full plan; near-duplicate scope.
- `docs/_plan/sphinx_subtask1_install_build_capture_plan.md` — third plan (separate authoring).
- `docs/_plan/sphinx_initial_build_subtask1_plan.md` (513 lines) — **the upstream input being consolidated here**.
- `docs/_plan/sphinx_warning_triage_plan_consolidated.md` — subtask-2 plan + post-execution verification report.
- `docs/_plan/sphinx_warning_triage_subtask2_deliverable.md` — subtask-2 deliverable artifact.
- `docs/_plan/sphinx_warning_triage_*` (multiple others) — subtask-2 working drafts.

This consolidated plan does **not** replace those — it is the *current-state-aware* execution document tailored to this specific phrasing of the request, citing the existing plans where they remain authoritative and diverging only where on-disk reality has shifted.

---

## 3. Discrepancies with the user request — and how the plan handles each

| # | Premise in request | Reality | Plan's handling |
|---|--------------------|---------|-----------------|
| 1 | "uv-native (per CONSOLIDATION_NOTES item #3 and pyproject.toml)" | None of those artifacts exist | §4 substitutes a project-local venv install path (or system Python — see Option 0); the **deliverable** (recorded versions + working `sphinx-build`) is unchanged |
| 2 | `uv add --dev sphinx sphinx-rtd-theme` | Would hard-fail with `error: No \`pyproject.toml\` found` | Substitute Option 0 (system Python, already installed) or Option C (`python -m venv .venv-docs` + `pip install`). Manifest captured via `pip freeze`/`pip show`. |
...
| 4 | "subtask 2 owns recovery" | The audit at `…/sphinx_scaffolding_audit.md` calls the build "subtask 3" — different numbering. Subtask 2 has *already* executed in this repo (post-fix log + consolidation exist) | This plan respects the **current request's numbering**: this run's output goes to subtask-2-of-current-spec. The fact that an earlier subtask 2 (different parent spec) has already executed is informational only. |
| 5 | "warm-build output is canonical" | True in general; on this docset `autosectionlabel_prefix_document = True` already mutes the most common cold/warm differential (duplicate labels). Toctree warnings are identical between cold/warm. | Plan still runs both builds and captures both verbatim, exactly as requested. Subtask 2 diffs them. |
| 6 | "if `sphinx-build` exits non-zero (build failure, not warning), capture the error and stop" | With `nitpicky = True`, warnings stay at exit 0 — the *expected* outcome here is exit 0 with high warning count | Plan codifies the "exit ≠ 0 ⇒ stop" rule in §7 failure matrix. Warnings alone do **not** trigger stop. |

None of these warrants halting before execution; all are accounting / install-path issues, not "the build cannot run."

---

## 4. Decision: how to install Sphinx (since `uv add` is not viable)

### 4.1 Hard constraints recapped
...
| # | Approach | Tradeoffs |
|---|----------|-----------|
| **0** | **Use system Python directly** (`/c/Users/yxinl/miniforge3/python` already has sphinx==8.2.3, sphinx-rtd-theme==3.0.2, docutils==0.21.2) | **Zero install step. Cheapest. Manifest captured via `pip show` / `pip freeze` against the same interpreter. Risk: not isolated; later `pip install` elsewhere could mutate versions. Fine for one-shot capture; less reproducible across machines.** |
...
**Two acceptable paths**, depending on the executor's priority:

- **Option 0** (system Python) — pick this if the goal is *just to produce the deliverable as fast as possible* and the executor accepts that the host's Python environment is the reproducibility boundary. This is the upstream plan's intent ("record exact versions") satisfied with **zero new files created at the repo root**.
- **Option C** (project-local venv) — pick this if the goal is *isolated, machine-independent reproducibility*. Adds `.venv-docs/` at repo root (which is not source/config and so does not violate the "do not modify existing files" constraint, but does add a new directory).

Both paths produce the same on-disk deliverables in `docs/build/`. The cheatsheet in §10 shows Option 0; if Option C is chosen, replace `python` → `.venv-docs/Scripts/python` and `sphinx-build` → `.venv-docs/Scripts/sphinx-build` in every command. Behavior is otherwise identical.

If the team strongly prefers uv-native and accepts the bootstrap step, Option B is interchangeable with Option C; only the install command changes.
...
- The two existing log files (`build-log-warm.txt`, `build-log-after-fix.txt`) are renamed before this run: `build-log-warm.txt` → `build-log-warm-pre-triage.txt`. This is a *file rename*, not a content modification of any `.rst` file or `conf.py` — it preserves prior-subtask provenance while freeing the canonical filename for this subtask's deliverable.
...
### 5.4 Why `rm -rf docs/build/html` (and not also `.doctrees`) between cold and warm

The user request is explicit: *delete the HTML output only*; keep the doctree cache so the second build is genuinely warm. Doctree cache persistence is what surfaces warm-only resolution warnings (when present). This plan honors that exactly in §6 Step 5.

---

## 6. Step-by-step execution

All commands assume the working directory is the repo root: `C:\Users\yxinl\OneDrive\Projects\PythonProjects\CoreProjects\OpenTeam`. **Bash syntax** (the platform's primary shell per CLAUDE.md). PowerShell variant in §6-bis.

The body below shows **Option 0** (system Python) as the recommended path, since the system interpreter already has every required package. To switch to **Option C** (project-local venv), prepend Step 2-bis below and replace `python` -> `.venv-docs/Scripts/python` and `sphinx-build` -> `.venv-docs/Scripts/sphinx-build` everywhere else.

### Step 1 — Preserve existing logs

```bash
# Rename the existing warm log so this subtask's log can land at the
# canonical name without overwriting prior-subtask evidence.
...
### Step 2 — Verify install (Option 0)

```bash
# Confirm the system interpreter has the three required packages.
python -c "import sphinx, sphinx_rtd_theme, docutils; \
print(f'sphinx={sphinx.__version__} sphinx_rtd_theme={sphinx_rtd_theme.__version__} docutils={docutils.__version__}')"

# Confirm sphinx-build is callable.
sphinx-build --version
```

**Expected:** one line with the three package versions, then `sphinx-build X.Y.Z`. Both exit 0.
**On failure:** drop to Step 2-bis (Option C) and proceed.

### Step 2-bis — Install into a project-local venv (Option C, only if Option 0 failed)
...
# Install Sphinx + theme; pin nothing — manifest captures whatever lands
...
**On failure** (network, dep conflict, no wheel): capture stdout+stderr to `docs/build/install-log.txt` and **stop** — hand off to subtask 2.

### Step 3 — Capture version manifest
...
mkdir -p docs/build
{
  ...
  pip show sphinx sphinx-rtd-theme docutils
  echo
  echo "--- pip freeze (full interpreter inventory) ---"
  pip freeze
...
# Sphinx's exit code.
sphinx-build -b html docs/source docs/build/html 2>&1 \
...
|-------------|---------|--------|
| 0 | Build succeeded; warnings recorded in log | proceed to Step 5 |
...
Expected count given current source state: **7** (`index, configuration, inference-models, operations, glossary, genindex, search`). Subtask 2 compares to expectation.
...
```powershell
# Step 4 (cold) — PowerShell, Option 0
...
For Option C in PowerShell, prefix the executable with `& .venv-docs/Scripts/sphinx-build.exe`.
...
|-------|---------------|--------|-------------------|
| 1. preserve | rename fails (file lock, permissions) | Stop. Don't proceed — would clobber prior log on Step 5. | error message; current `docs/build/` listing |
| 2. verify | `import sphinx` fails | Drop to Option C (Step 2-bis). | which-python output |
| 2-bis. install | venv creation fails | Stop. | `python -m venv` stderr; OS / Python version |
| 2-bis. install | `pip install` fails (network, no wheel, dep conflict) | Stop. | install stdout+stderr -> `docs/build/install-log.txt`; stop reason |
| 2/2-bis. verify | `sphinx-build --version` errors | Stop. | stderr; output of `python -c 'import sphinx; print(sphinx.__version__)'` if it works |
| 3. manifest | `pip show` / `pip freeze` errors | **Continue** — manifest is informational, not blocking. | whatever was captured |
| 4. cold | `sphinx-build` exit ≠ 0 | Stop. **Do not run warm build.** | cold log only; exit code; manifest |
| 4. cold | exit = 0 but `docs/build/html/index.html` missing | Treat as anomalous. Hand off both logs + directory listing. | cold log; html listing; manifest |
| 5. warm | `rm -rf docs/build/html` errors (lock, permission) | Stop. | error; current state of `docs/build/` |
| 5. warm | exit ≠ 0 (after cold succeeded) | Stop. **Flag explicitly** as warm-only resolution failure (rare and interesting). | both logs; both exit codes |
| 6. count | `find` returns 0 HTML files even with exit 0 | Treat as anomaly. Hand off everything. | both logs; empty html listing |
...
├── install-log.txt                        <- only if Step 2-bis ran AND pip install failed (else absent)
...
└── html/                                  <- canonical output from the warm build
    ...
    └── _static/, _sources/, objects.inv, searchindex.js
```

If Option C ran: also `.venv-docs/` at repo root.
...
- (Option C only) `.venv-docs/` at repo root (Python venv directory).
...
## 10. Quick-start cheatsheet (condensed copy-paste, Option 0)

```bash
# from repo root, bash, system Python (Option 0 — recommended given current state)
# 1. preserve existing logs
[ -f docs/build/build-log-warm.txt ] && \
  mv docs/build/build-log-warm.txt docs/build/build-log-warm-pre-triage.txt

# 2. verify install (Option 0 — packages already present in system Python)
python -c "import sphinx, sphinx_rtd_theme, docutils; \
print(f'sphinx={sphinx.__version__} sphinx_rtd_theme={sphinx_rtd_theme.__version__} docutils={docutils.__version__}')"
sphinx-build --version
...
For Option C, replace step 2 with `python -m venv .venv-docs && .venv-docs/Scripts/pip install sphinx sphinx-rtd-theme`, then prefix every subsequent `python` / `sphinx-build` / `pip` invocation with `.venv-docs/Scripts/`.

---

## 11. Open questions (non-blocking; surface in handoff if not resolved)

1. **Option 0 vs Option C** — given system Python already has the three required packages, is the executor authorized to skip venv creation (Option 0) or must it always isolate (Option C)? Plan defaults to Option 0 as cheapest; falls back to Option C if Option 0 verification fails.
2. **Is creating `.venv-docs/` at repo root acceptable** under the "do not modify existing files" framing (relevant only if Option C runs)? Plan reads the constraint as applying to source/config files, not a tooling venv. Belt-and-suspenders alternative: `docs/.venv/`.
3. **Pin versions or take latest?** User request says "record the exact installed versions … for the build report" — implies "whatever lands". Manifest captures whatever is present. Plan does not pin.
4. **If cold build fails, debug or just capture-and-stop?** Plan assumes strict capture-and-stop per user request's exact wording. If executor is authorized to *peek* (read traceback for handoff context), update handoff with findings — but **do not** edit any `.rst` or `conf.py`.
5. **Should `.venv-docs/` persist** for subtask 2 (Option C only)? Plan assumes persistence; subtask 2 will need it to re-run the build during triage.
6. **Are the two preserved logs (`build-log-warm-pre-triage.txt`, `build-log-after-fix.txt`) supposed to remain on disk** indefinitely, or is this subtask the right place to archive/delete them? Plan keeps them — they are evidence cited by the consolidation report. Decision deferred to subtask 2 / parent spec owner.
...
(Collapsed from upstream §1.5 and §11 to avoid duplication.)

- `docs/_plan/sphinx_initial_build_subtask1_plan.md` — the single upstream plan being consolidated here.
- `docs/_plan/sphinx_scaffolding_audit.md` — predicts toctree-missing warnings + read-only foundation status for `configuration.rst` / `inference-models.rst` / `operations.rst`.
- `docs/_plan/sphinx_warning_triage_plan_consolidated.md` — subtask 2 plan + post-execution verification (the report that confirms `build-log-after-fix.txt` shows 6 warnings = predicted).
- `docs/_plan/sphinx_warning_triage_subtask2_deliverable.md` — subtask 2 deliverable artifact.
