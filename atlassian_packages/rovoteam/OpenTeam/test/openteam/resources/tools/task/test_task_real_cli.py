...
# OpenTeam repo (the target_path for ClaudeCodeCli leaves to investigate)
OPENSTARTUP_PATH = Path(
    os.environ.get(
        "OPENSTARTUP_PATH",
        # default: this file at OpenTeam/test/openteam/resources/tools/task/
...
PAI_DOCS_OUTPUT_PATH = Path(
    os.environ.get(
        ...
        #   [0]=tools, [1]=resources, [2]=openteam, [3]=test, [4]=OpenTeam,
        #   [5]=CoreProjects. Reference docs live under
        #   OpenTeam/_dev/pai_hack/codebase_understanding (i.e. parents[4]).
...
async def test_real_pai_codebase_understanding_with_rovodev(tmp_path, capfd):
    ...
    overrides = {
        ...
        # Templates: still load from OpenTeam (the topology lives here).
        "templates_dir": str(TEMPLATES_DIR),
...
async def test_real_cli_subprocess_plan_mode(tmp_path, capfd):
    ...
    # _HERE is .../OpenTeam/test/openteam/resources/tools/task/
    # parents[4] = .../OpenTeam
    ...
    pythonpath_parts = [
        ...
        str(coreprojects_root / "PythonUtils" / "src"),
    ...
    # OpenTeam/src/openteam/server/_runtime/tasks/task_task-<hash>/
    runtime_tasks = (
