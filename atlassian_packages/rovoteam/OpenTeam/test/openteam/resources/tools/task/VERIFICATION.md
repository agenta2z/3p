...
2. Auto-discover latest workspace:
   ...
   export WS=$(ls -td /Users/tchen7/MyProjects/CoreProjects/OpenTeam/_runtime/tasks/task/task_* | head -1)
   echo "Auditing: $WS"
   ```
...
- **Source**: Documented inline in the topology YAML self-doc — `OpenTeam/src/openteam/server/resources/tools/task/topologies/breakdown-multiflow-plan.yaml` lines 16–26 ("DualInferencer behavior makes fixer = base_inferencer … ~10-20× more LLM calls than needed for typical plan-quality feedback"). The lightweight-fixer wiring exists specifically to prevent regression.
...
- **Source**: `OpenTeam/test/openteam/resources/tools/task/preflight/test_plan_mode_yaml_swap.py` (preflight regression test) — guards `executor._run_topology` swap from `breakdown-multiflow-plan-then-implement.yaml` → `breakdown-multiflow-plan.yaml` when `mode == "plan"`.
