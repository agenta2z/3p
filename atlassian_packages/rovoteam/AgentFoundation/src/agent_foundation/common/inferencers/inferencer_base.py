...
from python_utils.common_objects.workflow.common.resumable import Resumable
from python_utils.common_utils import dict_, iter__, resolve_environ
from python_utils.common_utils.function_helper import FallbackMode, execute_with_retry
from python_utils.path_utils import AllowedPath, PathAccess
...
class InferencerBase(Debuggable, Resumable, ABC):
    ...
    def _redirect_loggers_to_workspace(self, workspace):
        ...
        import os
        from python_utils.io_utils.json_io import JsonLogger
    ...
    def __attrs_post_init__(self):
        ...
        if isinstance(self.post_response_merger, str):
            if self.post_response_merger == "default":
                from python_utils.mp_utils.common import merge_results

                self.post_response_merger = merge_results
            else:
                from python_utils.mp_utils.common import get_merger
    ...
    def _add_workspace_logger(self, workspace):
        ...
        from python_utils.io_utils.json_io import JsonLogger, SpaceExtMode
        from python_utils.common_objects.debuggable import LoggerConfig
    ...
    def parallel_infer(
        ...
        **_inference_args,
        ...
        """
        try:
            from python_utils.mp_utils.mp_target import MPTarget
            from python_utils.mp_utils.parallel_process import parallel_process_by_pool
        except ImportError as _e:
            raise NotImplementedError(
                "parallel_infer requires python_utils.mp_utils which is not "
                "available in this environment. Install python_utils with the "
        ...
        if num_workers is None:
            if use_threading:
                ...
                from python_utils.mp_utils.common import get_suggested_num_workers
    ...
    async def __ainfer_single_impl(
        ...
        **_inference_args,
        ...
        """
        from python_utils.common_utils.async_function_helper import (
