...
from python_utils.string_utils.formatting.template_manager.sop_manager import SOPManager
...
class ConversationalInferencer(InferencerBase):
    ...
    def _render_prompt(self, current_message: str) -> str:
        ...
        if sop_path is not None:
            try:
                from python_utils.common_objects.workflow.stategraph import (
                    StateGraphTracker,
                )
                ...
                if self.prior_context.pop("_confirmation_gate_passed", False):
                    from python_utils.string_utils.formatting.template_manager.sop_manager import SOPPhase
