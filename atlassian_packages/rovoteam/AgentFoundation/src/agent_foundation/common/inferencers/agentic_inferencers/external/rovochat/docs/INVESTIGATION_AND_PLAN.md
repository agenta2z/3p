...
Uses `get__()` from `python_utils.common_utils.map_helper` for multi-key lookups.
...
- `python_utils` — `get__()` for multi-key env var lookup; `_run_async()` for sync wrapper
...
| Dependencies hard or soft? | `httpx` is soft (ImportError at runtime). `atlassian-jwt-auth` is soft (only for ASAP mode). `python_utils` and `attrs` are hard (framework deps). |
