...
External dependencies:
  ...
  ├── python_utils.get__   (hard — in auth.py, rovochat_inferencer.py)
...
- Uses `_run_async()` from `python_utils` for sync wrapper
- Uses `get__()` from `python_utils` for multi-key env var lookups
