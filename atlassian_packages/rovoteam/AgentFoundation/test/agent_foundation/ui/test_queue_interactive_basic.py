...
Prerequisites:
    python_utils package installed
...
# Also add python_utils to path if available
python_utils_path = project_root.parent / 'SciencePythonUtils' / 'src'
...
try:
    ...
    from python_utils.service_utils.queue_service.thread_queue_service import (
        ThreadQueueService
    )
    QUEUE_SERVICES_AVAILABLE = True
except ImportError:
    QUEUE_SERVICES_AVAILABLE = False
    print("Warning: python_utils not available, skipping tests")
...
""")
    ...
    if not QUEUE_SERVICES_AVAILABLE:
        print("\n[ERROR] python_utils not available. Cannot run tests.")
        print("Please ensure python_utils is installed and in PYTHONPATH.")
        sys.exit(1)
