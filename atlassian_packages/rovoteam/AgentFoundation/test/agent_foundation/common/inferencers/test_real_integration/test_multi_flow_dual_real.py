"""Real-CLI integration test — MultiFlowDualInferencer documents OpenTeam.
...
- Real `target_path` pointing at the OpenTeam repo.

The task: produce an architecture-overview document for OpenTeam. Loose
...
# ---------------------------------------------------------------------------
# OpenTeam discovery (env override or default sibling-of-AgentFoundation)
# ---------------------------------------------------------------------------

OPENSTARTUP_PATH = Path(
    os.environ.get(
        "OPENSTARTUP_PATH",
        # default: AgentFoundation/test/.../test_real_integration → up 6 → CoreProjects/OpenTeam
        str(Path(__file__).resolve().parents[6] / "OpenTeam"),
    )
)

skip_openstartup = pytest.mark.skipif(
    not OPENSTARTUP_PATH.exists(),
    reason=f"OpenTeam repo not found at {OPENSTARTUP_PATH}",
)
...
async def test_multi_flow_dual_documents_openstartup(tmp_path):
    """Real Claude+Kiro composition: produce architecture overview for OpenTeam.
    ...
    Skipped automatically when ``claude``, ``kiro-cli``, or OpenTeam
    are unavailable.
    """
    ...
    DOC_TASK = (
        "You are creating a documentation PLAN for the OpenTeam codebase "
    ...
    async with mfdi:
        result = await mfdi.ainfer(
            "Document the OpenTeam codebase architecture"
        )
