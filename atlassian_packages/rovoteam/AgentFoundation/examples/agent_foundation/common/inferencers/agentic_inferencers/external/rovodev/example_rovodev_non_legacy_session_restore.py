#!/usr/bin/env python3
"""Rovo Dev CLI Inferencer — Non-Legacy Mode Session Save & Restore Demo.

Demonstrates that Rovo Dev sessions can be saved and restored by their
UUID in non-legacy (TUI) mode, enabling true multi-turn conversations
...
This uses ``acli rovodev --restore <uuid>`` (non-legacy mode) with
auto-injected ``--output-schema`` for clean output capture.
...
Usage:
    python example_rovodev_non_legacy_session_restore.py
    python example_rovodev_non_legacy_session_restore.py --working-dir /path/to/repo
"""
...
# Auto-add AgentFoundation/src and PythonUtils/src to path
...
_rich_utils_src = os.path.normpath(
    os.path.join(_agent_foundation_root, "..", "PythonUtils", "src")
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Rovo Dev CLI Inferencer — Non-Legacy Session Restore Demo",
    ...
    if args.working_dir:
        ...
        working_dir = tempfile.mkdtemp(prefix="rovodev_nonlegacy_session_")
        subprocess.run(
    ...
    print()
    print("Rovo Dev Session Save & Restore Demo (Non-Legacy Mode)")
    ...
    try:
        ...
        # --- Session A ---
        inferencer_a = create_inferencer(working_dir)
        ...
        # --- Session B ---
        inferencer_b = create_inferencer(working_dir)
        ...
        inferencer_resume_a = create_inferencer(working_dir)
        ...
        inferencer_resume_b = create_inferencer(working_dir)
        ...
        print()
        print("=" * 70)
        print("Manual session restore commands (non-legacy)")
        print("=" * 70)
        print()
        print(f"  # Resume Session A:")
        print(f"  acli rovodev --restore {session_a_id}")
        print()
        print(f"  # Resume Session B:")
        print(f"  acli rovodev --restore {session_b_id}")
        print()
